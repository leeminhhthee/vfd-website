...
export default function RelatedTournaments({
  tournaments,
}: RelatedTournamentsProps) {
  if (tournaments.length === 0) return null;

  return (
    <div className="mt-12 bg-white rounded-lg border border-border p-6">
      <h2 className="text-2xl font-bold text-foreground mb-6">
        {trans.relatedTournaments}
      </h2>
          ...
          <Link key={tournament.id} href={`/schedule/${tournament.id}`}>
            <div className="p-4 m-2 border border-border rounded-lg hover:bg-muted transition-colors cursor-pointer">
              ...
              <div className="flex justify-between items-center text-xs text-muted-foreground">
                <span>
                  {format(
                    parseISO(tournament.startDate?.toString()),
                    "dd/MM/yyyy",
                    { locale: vi }
                  )}
                </span>
                <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs font-semibold">
                  {getScheduleStatusLabel(tournament.status)}
                </span>
              </div>
            </div>
          </Link>
